...
def load_wrapper(Data, Settings):
    ...
    if Settings['File Type'] == 'LCPro':
        ...
        try:
            # Load the 'ROI normalized.txt' file
            data_path = r'%s/ROI normalized.txt' % folder
            data = pd.read_csv(data_path, index_col='time(s)', sep='\t')
            Data['original'] = data
            print(f"Loaded time series from: {data_path}")
        except IOError:
            # Handle file not found error
            print(f"File 'ROI normalized.txt' not found in the specified folder: {folder}")
            raise
    
        try:
            # Load additional ROI parameters
            roi_param_path = r'%s/Parameter List.txt' % folder
            roi_param = pd.read_csv(roi_param_path, index_col=0, sep='\t')
            Data['ROI parameters'] = roi_param
            print(f"Loaded Centroids from: {roi_param_path}")
        except IOError:
            print(f"File 'Parameter List.txt' not found in the specified folder: {folder}")
            raise
    
        try:
            # Load the image
            image_path = r'%s/rgb.png' % folder
            im = Image.open(image_path)
            print(f"Loaded 'rgb.png' from: {image_path}")
        except IOError:
            print(f"Image 'rgb.png' not found in the specified folder: {folder}")
            raise

        # Update ROI parameters index
        new_index = ['Mean' + str(idx) for idx in roi_param.index]
        Data['ROI parameters'].index = new_index
        ...
        try:
            i = 0
            while six.text_type(pd.read_csv('%s/%s' %(Settings['folder'], Settings['Label']), sep = '\t', 
    ...
    #HDF5, from RTXI, time in ms
        ...
        def getData(f):
            ...
            for i in headers:
                if i != headers[chan-1]:
                    del dframe[i]
            dframe["Time (ms)"] = list(range(0, len(dframe))) #Establish abscissa
            return dframe
...
def load_settings(Settings):
    try:
        # Try loading the CSV without specifying an index column
        settings_temp = pd.read_csv(Settings['Settings File'], header=0, sep=',')
    ...
    if settings_temp.empty:
        print("Settings DataFrame is empty.")
        return Settings
    
    # Print DataFrame shape and columns
    print(("DataFrame shape:", settings_temp.shape))
    print(("Column names:", settings_temp.columns))

    # Ensure DataFrame has two columns
    if len(settings_temp.columns) != 2:
        print("Expected DataFrame to have exactly two columns.")
        return Settings

    # Process the DataFrame
    exclusion_list = ['plots folder', 'folder', 'Sample Rate (s/frame)', 'Output Folder', 
                      'Baseline', 'Baseline-Rolling', 'Settings File', 'Time Scale',
                      'Label', 'File Type', 'HDF Key', 'HDF Channel']
    
    for row in settings_temp.itertuples(index=False):
        key = row[0].strip()  # First column
        val = row[1].strip()  # Second column
...
def display_settings(Settings):
    ...
    if 'Baseline-Rolling' in list(Settings_copy.keys()):
    ...
    Settings_copy = Settings_copy.sort_values(by='Value')
...
def savitzky_golay(y, window_size, order, deriv=0, rate=1):
    ...
    if y is None:
        raise ValueError("Input data y cannot be None")
    ...
    if window_size < order + 2:
        raise ValueError("window_size is too small for the polynomial's order")
...
def user_input_trans(Settings):
    ...
    if 'Linear Fit' in list(Settings.keys()):
    ...
    if 'Bandpass Lowcut' in list(Settings.keys()):
    ...
    if 'Savitzky-Golay Window Size' in list(Settings.keys()):
    ...
    if window_sav != 'none':
        ...
        if 'Absolute Value' in list(Settings.keys()):
...
def linear_subtraction(data, time_array, R_raw, R_roll, window, b):
    '''
    Perform linear subtraction using rolling mean and fit parameters.
    ...
    R_raw: bool
        Raw fit parameter (not used in this example)
    R_roll: bool
        Rolling fit parameter (not used in this example)
    window: int
    ...
    -------
    ls_tf: 1d array
        Linearly subtracted data
    '''
    # Convert data to numpy array if not already
    data = np.asarray(data, dtype=float)
    
    # Check if window is an integer and is valid
    if not isinstance(window, int):
        raise TypeError(f"Expected an integer for window size, got {type(window).__name__}")
    
    if window <= 0 or window > len(data):
        raise ValueError(f"Invalid window size: {window}. Must be > 0 and <= {len(data)}")

    # Compute rolling mean
    rolling_mean, _, _ = baseline_rolling(time_array, data, window)
    
    if rolling_mean.size == 0:
        raise ValueError("rolling_mean is empty. Check the baseline_rolling function.")
    
    # Check length of rolling mean
    if len(rolling_mean) > len(data):
        raise ValueError("Rolling mean length exceeds data length.")
    
    # Ensure rolling_mean and b are numeric
    if not np.issubdtype(rolling_mean.dtype, np.number):
        raise TypeError(f"Expected numeric type for rolling_mean, got {rolling_mean.dtype}")
    
    if not isinstance(b, (float, int)):
        # Try to convert b to a float if it's a string
        try:
            b = float(b)
        except ValueError:
    ...
    try:
        ...
        ls_tf = data[:len(rolling_mean)] - (rolling_mean + b)
    except ValueError as e:
        print(f"Data length: {len(data)}, Rolling mean length: {len(rolling_mean)}")
        print(f"rolling_mean dtype: {rolling_mean.dtype}, b type: {type(b)}")
        raise e
...
def transformation(data, Settings):
    '''
    Apply transformations to the data based on the given settings.
    ...
    Settings: dict
        Dictionary of settings to be applied
    
    Returns
    -------
    transformed_data: DataFrame or Series
        Transformed data
    '''
    # Extract necessary settings
    linear_fit = Settings.get('Linear Fit', False)
    linear_fit_rolling_r = Settings.get('Linear Fit-Rolling R', 0)
    linear_fit_rolling_window = Settings.get('Linear Fit-Rolling Window', 1)
    relative_baseline = Settings.get('Relative Baseline', 0)
    
    # Convert settings to appropriate types
    if isinstance(linear_fit_rolling_window, str):
        try:
            linear_fit_rolling_window = int(linear_fit_rolling_window)
        except ValueError:
            raise ValueError(f"Invalid value for 'Linear Fit-Rolling Window': {linear_fit_rolling_window}")

    if isinstance(relative_baseline, str):
        try:
            relative_baseline = float(relative_baseline)
        except ValueError:
            raise ValueError(f"Invalid value for 'Relative Baseline': {relative_baseline}")

    # Apply linear subtraction if specified
    if linear_fit:
        # Convert 'data' to a numpy array for compatibility
        data_trans = np.asarray(data, dtype=float)
        time_array = np.arange(len(data_trans))  # Create a time_array assuming it's sequential
        data_trans = linear_subtraction(data_trans, time_array,
                                        linear_fit,
                                        linear_fit_rolling_r,
                                        linear_fit_rolling_window,
                                        relative_baseline)
        # Debugging: Check data after linear subtraction
        print("Data after linear subtraction:")
        print(data_trans)
    else:
        data_trans = data
...
def transform_wrapper(Data, Settings):
    ...
    """
    Data['trans'] = pd.DataFrame(index=Data['original'].index)
    
    for label, column in Data['original'].items():
        # Apply transformations
        data_trans = transformation(column, Settings)
        
        # Store transformed data in 'trans' key
...
def user_input_base(Settings):
    ...
    if 'Baseline Type' in list(Settings.keys()):
    ...
    if baseline_type == '': #defaults to static
        ...
        Settings['Baseline Type'] = baseline_type
        if 'Rolling Baseline Window' in list(Settings.keys()):
...
def baseline_wrapper(Data, Settings, Results):
    ...
    if 'Baseline Type' not in list(Settings.keys()):
    ...
    if Settings['Baseline Type'] == 'static':
        ...
        for label, column in six.iteritems(Data['trans']): #the problem with aligned time series is that the baserate calculation will be performed for each one, not garunteeing a good shift.
        ...
        for label, column in six.iteritems(Data['trans']):
...
def baseline_rolling(time_array, data, window):
    '''
    Compute the rolling baseline for the given data.
    
    Parameters
    ----------
    time_array: 1d array
        Contains time values in seconds
    data: 1d array
        Contains y values to be processed
    window: int
        Window size for rolling average
    
    Returns
    -------
    rolling_mean: 1d array
        The rolling mean used as baseline
    data_roll: 1d array
        Data adjusted by rolling mean (if applicable)
    time_roll: 1d array
        Time array adjusted to match the rolling mean
    '''
    ...
    rolling_mean[np.isnan(data)] = np.nan
    
    # Debugging information
    print(f"Rolling mean (first 10 values): {rolling_mean[:10]}")
    print(f"Length of rolling mean: {len(rolling_mean)}")
    
    return rolling_mean, data, time_array
...
def peakdet(v, delta, x=None):
    """
    Detects peaks in a vector v. Returns two arrays:
    - maxtab: Local maxima (peaks)
    - mintab: Local minima (valleys)

    Args:
    v: Input vector.
    delta: Minimum value difference between a peak and its surrounding points to be considered a peak.
    x: Optional argument. X-values corresponding to the indices in v. If None, indices are used.

    Returns:
    Tuple of two arrays (maxtab, mintab). Each array consists of two columns. Column 1 contains indices in v, and column 2 contains the found values.
    """
    ...
    v = np.asarray(v, dtype=float)  # Convert v to float
    ...
    try:
        delta = float(delta)  # Ensure delta is a float
    except ValueError:
        raise ValueError('Input argument delta could not be converted to float')
    ...
    lookformax = True  # Initialize to True to start looking for maxima
...
def event_peakdet_settings(Data, Settings):
    ...
    '''
    if 'Delta' in list(Settings.keys()):
    ...
    if 'Peak Minimum' in list(Settings.keys()):
    ...
    if 'Peak Maximum' in list(Settings.keys()):
...
def event_peakdet(Data, Settings, Results, roi):
    '''
    Wrapper that directs one column of data, defined by roi name into the correct post processing from peak and valley detection.
    '''
    failure = False
    if Settings['Baseline Type'] == 'linear':
        maxtab, mintab = peakdet(np.array(Data, dtype=float), Settings['Delta'], None)
        
        if maxtab.size == 0:
            maxptime = [np.nan]
            maxpeaks = [np.nan]
            results_peaks = pd.DataFrame({'Peaks Amplitude': maxpeaks, 'Intervals': maxpeaks})
            print(roi, 'has no peaks.')
        else:
            maxtab = np.array(maxtab)
            maxptime = maxtab[:,0]
            maxptime_true = (np.multiply(maxptime, Settings['Sample Rate (s/frame)']) + Data.index[0])
            maxpeaks = maxtab[:,1] - Results['Baseline'].get(roi, 0)
            results_peaks = pd.DataFrame({'Peaks Amplitude': maxpeaks}, index=maxptime_true)
            results_peaks = results_peaks[(results_peaks['Peaks Amplitude'] > Settings['Peak Minimum']) & (results_peaks['Peaks Amplitude'] <= Settings['Peak Maximum'])]
            if results_peaks.empty:
                failure = True
                print(roi, 'has no peaks.')
            else:
        ...
        if mintab.size == 0:
            results_valleys = pd.DataFrame({'Valley Amplitude': [np.nan]})
        else:
            mintab = np.array(mintab)
            valleytime = mintab[:,0]
            valleytime = (np.multiply(valleytime, Settings['Sample Rate (s/frame)']) + Data.index[0])
            valleys = mintab[:,1] - Results['Baseline'].get(roi, 0)
            results_valleys = pd.DataFrame({'Valley Amplitude': valleys}, index=valleytime)

    elif Settings['Baseline Type'] in ['rolling', 'static']:
        maxtab, mintab = peakdet(np.array(Data, dtype=float), Settings['Delta'], None)
        if maxtab.size == 0:
            maxptime = [np.nan]
            maxpeaks = [np.nan]
...
def event_peakdet_wrapper(Data, Settings, Results):
    '''
    The wrapper that handles applying peak detection on an entire DataFrame.
    '''
    ...
    try:
        ...
        Results['Peaks-Master'] = pd.DataFrame()
        print("No Peaks Found in any time series. Error: ", e)

    try:
        master_valleys = pd.concat(Results['Valleys'])
        Results['Valleys-Master'] = master_valleys
    except Exception as e:
        Results['Valleys-Master'] = pd.DataFrame()
        print("No Valleys Found in any time series. Error: ", e)
...
def event_burstdet_settings(Data, Settings, Results):
    ...
    #Threshold
    if 'Threshold' in list(Settings.keys()):
    ...
    #inter-event min
    if 'Inter-event interval minimum (time-scale units)' in list(Settings.keys()):
    ...
    #burst duration min
    if 'Minimum Burst Duration (time-scale units)' in list(Settings.keys()):
    ...
    #burst duration max
    if 'Maximum Burst Duration (time-scale units)' in list(Settings.keys()):
    ...
    #Minimum Peak Num
    if 'Minimum Peak Number' in list(Settings.keys()):
    ...
    #Burst Area
    if 'Burst Area' in list(Settings.keys()):
    ...
    #Exclude edges
    if 'Exclude Edges' in list(Settings.keys()):
...
def event_burstdet_wrapper(Data, Settings, Results):
    ...
    for roi, col in six.iteritems(data_temp):
...
def Save_Results(Data, Settings, Results):
    ...
    print("-----")
    for label, col in six.iteritems(Results['Peaks-Master']):
        print(label)
    print("\n--------------------------------------------")
    print("Available Measurements from Bursts for further analysis:")
    print("-----")
    for label, col in six.iteritems(Results['Bursts-Master']):
        print(label)
...
def results_timeseries_plot(roi, start, end, Data, Settings, Results):
    ...
    # Create plot for transformed data
    ...
    try:
        if not Results['Peaks'][roi].empty:
    ...
    try:
        if not Results['Valleys'][roi].empty:
    ...
    try:
        if Settings['Baseline Type'] == 'static':
            start_y = [Settings['Threshold']] * len(Results['Bursts'][roi]['Burst Start'])
            end_y = [Settings['Threshold']] * len(Results['Bursts'][roi]['Burst End'])
            ...
            start_y = [Results['Baseline'][roi] * Settings['Threshold']] * len(Results['Bursts'][roi]['Burst Start'])
            end_y = [Results['Baseline'][roi] * Settings['Threshold']] * len(Results['Bursts'][roi]['Burst End'])
            f2.plot(Results['Bursts'][roi]['Burst Start'], start_y,
                    marker='s', color='g', linestyle='None', alpha=1, label='Burst Start', markersize=5)
            f2.plot(Results['Bursts'][roi]['Burst End'], end_y,
                    marker='s', color='y', linestyle='None', alpha=1, label='Burst End', markersize=5)
            ...
            f2.plot(Results['Bursts'][roi]['Burst End'], Results['Bursts'][roi]['Burst End Amplitude'],
                    marker='s', color='y', linestyle='None', alpha=1, label='Burst End', markersize=5)
            f2.plot(Results['Baseline-Rolling'][roi].index, Results['Baseline-Rolling'][roi], 'b')  # Plot rolling baseline
    ...
    try:
        if Settings['Baseline Type'] == 'linear': 
            f2.hlines(Results['Baseline'][roi] * Settings['Threshold'], Data['original'].index[0], 
                      Data['original'].index[-1], 'k', label='Baseline')
        elif Settings['Baseline Type'] == 'rolling':
            f2.plot(Results['Baseline-Rolling'][roi].index, Results['Baseline-Rolling'][roi], 'b')  # Plot rolling baseline
    except KeyError:
        pass
...
def graph_ts(Data, Settings, Results, roi = 'random', mode = 'display', lcpro = False):
    ...
    try:
        if Settings['Baseline Type'] == 'static':
            ...
            data_temp = Data['rolling']
            plt.ylabel('Relative Amplitude (Transformed Data)')
    except Exception: #if no baseline type is set
        try: #try using data trans
            data_temp = Data['trans']
            plt.ylabel('Relative Amplitude (Transformed Data)')
        except Exception: #if not yet transformed, used original
    ...
    except Exception:#baseline has not yet been set
        pass
    ...
    except Exception:#threshold has not yet been set
        pass
    ...
    except Exception:#this would be the case that no bursts were detected
        pass
...
def average_measurement_plot(event_type, meas, Results):
    ...
    try:
        ...
        plt.show()
    except Exception: #this occurs when the column names are strings that cannot be converted into floats
...
def raster(Data, Results):
    ...
    """
    for key, value in six.iteritems(Results['Peaks']):
        try:
            key = float(key)
        except Exception:
...
def poincare_batch(event_type, meas, Data, Settings, Results):
    ...
    if 'Poincare SD1' not in list(Results.keys()):
    ...
    if meas.lower() == 'all':
        ...
        for key, value in six.iteritems(measurement):
...
def psd_signal(version, key, scale, Data, Settings, Results):
    ...
    if 'PSD-Signal'not in list(Results.keys()):
...
def psd_event(event_type, meas, key, scale, Data, Settings, Results):
    ...
    try:
        if 'PSD-Event'not in list(Results.keys()):
            Results['PSD-Event'] = {}
        
        if key not in list(Results['PSD-Event'].keys()):
...
def histent_wrapper(event_type, meas, Data, Settings, Results):
    ...
    if 'Histogram Entropy' not in list(Results.keys()):
...
def ap_entropy_wrapper(event_type, meas, Data, Settings, Results):
    ...
    if 'Approximate Entropy' not in list(Results.keys()):
...
def samp_entropy_wrapper(event_type, meas, Data, Settings, Results):
    ...
    if 'Sample Entropy' not in list(Results.keys()):
...
def moving_statistics(event_type, meas, window, Data, Settings, Results):
    ...
    if 'Moving Stats' not in list(Results.keys()):
...
def analyze(Data, Settings, Results):
    ...
    #Load    
    print("Contents of Data dictionary before analyze function:")
    print(Data)

    print("Keys in Data dictionary:")
    print((list(Data.keys())))

    print(("Is 'original' in Data?", 'original' in Data))

  
    if len(Data['original'].columns) == 1:
        Data['original'].columns = [Settings['Label']]
    else:
        Data['original'].columns = ['%s_%d' % (Settings['Label'], i+1) for i in range(len(Data['original'].columns))]
    ...
    if Settings['Generate Graphs'] == True:
        for label, col in six.iteritems(Data['original']):
